...
int main(int argc, char *argv[])
{
	...
	int i, j, k, m, waitTime[20], turnAroundTime[20], process[20];
	...
	if (maxBurstTime > SMALLSLICER) {
        for (j = 0; j < (maxBurstTime / BIGSLICER) + 1; j++)  {   

		for (i = 0; i < numProcesses; i++)  { 

			if (burstTime[i] != 0) {                     // as long as burst time is not 0
                
				if (burstTime[i] < BIGSLICER) {         // if burst time is smaller than time slice
				...
				else if (burstTime[i] > SMALLSLICER) {     // if burst time is larger than time slice

					burstTime[i] = burstTime[i] - BIGSLICER;             // current burst time = burst time - time slice
					arranged = arranged + BIGSLICER;                     // set arranged value = last arranged value + time slice
				}
	...
	for (int k = 0; k < numProcesses; k++) {
		if (k == 0) {
            ...
            if (burstTime[i] < BIGSLICER)
            {
                responseTime[k] = (k * BIGSLICER) - burstTime[k - 1];
        	    totalResponseTime += responseTime[k]; 
            }
            else {
                responseTime[k] = (k * SMALLSLICER) - burstTime[k - 1];
        	    totalResponseTime += responseTime[k]; 
            }
